# 第6章

## 6.1 定积分的概念

### 一、曲边梯形的面积

### 二、变速直线运动的路程

### 三、变密度直线段构件的质量

## 6.2 可积函数与定积分的性质

## 6.3 微积分基本公式

## 6.4 定积分的换元积分法与分部积分法

### 一、定积分的换元积分法

1、求定积分： $\int ^3_0 \arcsin \sqrt{\frac{x}{x+1}}dt$ .  
令 $\arcsin \sqrt{\frac{x}{x+1}}=t$ ， 则 $x=\tan^2t$  
通过计算得到答案为： $\frac{4\pi}{3}-\sqrt{3}$  

### 二、定积分的分部积分法

### 三、变上限积分的导数与定积分不等式

1、设 $f$ 在 $[a,b]$ 上连续，且 $f(x) \gt 0$ .证明：曲线

$$
F(x)=\int ^b_a |x-t|f(t)dt
$$

在 $[a,b]$ 上为向上凹的。

证明：
将原式拆分进行去绝对值。

$$
F(x)=\int ^x_a (x-t)f(t)dt-\int ^b_x (x-t)f(t)dt
$$

对其求二阶导易得其恒大于 $0$ ；即为向上凹。

2、设 $f$ 在 $[a,b]$ 上连续，且 $f(x) \gt 0.$ 证明：

$$
\int ^b_a f(x)dx \centerdot \int ^b_a \frac{dx}{f(x)} \ge (b-a)^2
$$

证明：
由**柯西不等式**：

$$
(\int ^b_a f(x)g(x)dx)^2 \le \int ^b_a f^2(x)dx \centerdot \int ^b_a g^2(x)dx.
$$

易证。

3、利用 $\Gamma$ 函数计算积分
$\int ^{+\infty}_0 x^6 e^{-2x}dx$  

解：
**$\Gamma$ 函数**：
表达式： $\Gamma (x) =\int ^{+ \infty}_0 x^{s-1}e^{-x}dx,(s \lt 0).$  
递推公式： $\Gamma(s+1) = s\Gamma(s),(s \lt 0)$  
特例： $\Gamma(n+1) = n!$  

由此得知答案为： $720$



## 6.5 反常积分

### 一、无限区间上的反常积分

### 二、无界函数的反常积分

### 三、反常积分的敛散性判别法

1、设 $a \gt b \gt 0,$ 反常积分 $\int ^{+\infty}_0 \frac{1}{x^a+x^b}dx$ 收敛，则().  
A. $a \gt 1$ 且 $b \gt 1$  
B. $a \gt 1$ 且 $b \lt 1$  
C. $a \lt 1$ 且 $a+b \gt 1$  
D. $a \lt 1$ 且 $b \lt 1$  

解：注意到要考虑 $x=0$ 和 $x \to +\infty$ 两种情况，因此要将其拆分。  
最后容易得到答案为： $B$ .

## 6.6 定积分的应用

### 一、定积分的应用基础——微元分析法

### 二、平面图形的面积

### 三、已知截面面积求体积 旋转体的体积

1、设函数 $f(x)$ 的定义域为 $(0,+\infty)$ ，且满足 $2f(x)+x^2f(\frac{1}{x})=\frac{x^2+2x}{\sqrt{1+x^2}}.$ 求 $f(x),$ 并求曲线 $y=f(x),$ 直线 $y=\frac{1}{2},y=\frac{\sqrt{3}}{2}$ 及 $y$ 轴所围图形绕 $x$ 轴旋转一周所成旋转体的体积。

解：  
**旋转体体积计算**：  
$V_x=\pi \int ^b_a y^2 dx,$  
$V_y=2\pi \int ^b_a x|y|dx.$  

由此易得答案为： $\frac{\pi ^2}{6}$


### 四、平面曲线的弧长和旋转曲面的侧面积

1、求曲线 $\begin{cases}​x = e^t + e^{-t}, \\​ y = 2t​\end{cases} (0 \le t \le 1)$ 绕 $x$ 轴旋转一周所得旋转体的侧面积。

解：
侧面积计算公式：

$$
S=\int ^a_b 2\pi y ds
$$

代入易得答案为： $8\pi (1-\frac{1}{e})$  

### 五、定积分在物理上的应用
